...
## 2024-06-07 - Added Input Length Limits for TextFields in Authentication
**Vulnerability:** The email and password TextFields on the Login and Signup screens were unbounded, lacking `maxLength` limits. This missing restriction could allow malicious users or bots to submit extremely large strings, leading to excessive memory allocation and potentially causing a client-side Denial of Service (DoS) or application crash.
**Learning:** Even standard input fields that rely on backend validation must have front-line defenses (like `maxLength`) to protect the client application's memory and performance from unusually large payloads.
**Prevention:** Always define reasonable `maxLength` limits for user inputs in UI forms, especially for standard fields like email addresses or passwords, to mitigate the risk of unbounded input processing.
