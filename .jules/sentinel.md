...
## 2024-06-12 - Prevented Database Implementation Leak in FirestoreService
**Vulnerability:** The application was not catching exceptions when interacting with Firestore (`createPaymentIntent`, `saveTrainingPlan`, `getTrainingPlan`). In case of a database error, unhandled exceptions from the Firebase SDK could potentially leak internal database structures, paths, or query logic to upstream error handlers, crash logs, or the user interface.
**Learning:** Raw errors from backend services like Firestore often contain internal details (e.g., table/collection names, failed constraints). Direct exposure of these exceptions violates the "Fail Securely" principle and can provide attackers with insights into the database schema.
**Prevention:** Always wrap direct database interactions in `try/catch` blocks. Do not use `rethrow` unless the exception is sanitized higher up. Instead, catch the raw exception and throw a generic, application-level exception (e.g., `Exception('Operation failed.')`) to inform the application state without leaking sensitive internal details.
