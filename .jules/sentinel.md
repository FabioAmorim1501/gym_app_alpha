...
## 2024-06-25 - Unhandled exceptions in Firestore operations exposing internals
**Vulnerability:** The `FirestoreService` was interacting directly with Firestore methods (`set` and `get`) without try-catch blocks. If these operations failed (e.g., due to permission errors or network issues), unhandled exceptions could be thrown up the call stack. This might lead to unhandled exception logging that leaks sensitive details about internal database paths, schema structures, or cloud infrastructure configurations to error trackers or server logs.
**Learning:** Raw stack traces and error details from database libraries like `cloud_firestore` can expose significant backend architectural details, creating an information disclosure risk.
**Prevention:** Always wrap direct database interactions in try-catch blocks. Catch the specific library exception (or general exceptions) and substitute it by throwing a generic application-level exception, such as `throw Exception('Failed to perform database operation.');`, to safely signal failure without leaking internal data points.
