...
## 2024-06-25 - Prevented Leaking of Database Structure in FirestoreService
**Vulnerability:** The `FirestoreService` was allowing `FirebaseException` to bubble up from its database interaction methods (`createPaymentIntent`, `saveTrainingPlan`, `getTrainingPlan`). If unhandled upstream or logged improperly, this could expose raw stack traces detailing the backend database structure and configuration.
**Learning:** External backend SDK calls, especially database queries and writes, can throw exceptions containing internal system paths or collection names. Letting these bubble up exposes implementation details.
**Prevention:** Always wrap direct database interactions in `try/catch` blocks and throw a generic application-level `Exception` (e.g., `Exception('Failed to retrieve training plan.')`) to obscure sensitive internal structure from potentially public error logs or crash reports.
