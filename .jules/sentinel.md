...
## 2024-06-25 - Missing Input Length Validation on Auth Screens
**Vulnerability:** The email and password TextFields on both Login and Signup screens lacked `maxLength` constraints. This could potentially allow an attacker or a malicious script to paste extremely large strings into the fields, consuming excessive memory and causing a client-side Denial of Service (DoS) or UI freezing.
**Learning:** Even simple authentication forms are vulnerable to resource exhaustion if user input is unbounded. Flutter's `TextField` should always have a reasonable limit to prevent abusive input from impacting performance or crashing the application.
**Prevention:** Consistently apply `maxLength` properties to all `TextField` widgets based on the expected input size (e.g., 50 for emails, 128 for passwords) to cap memory allocation and prevent DoS risks.
